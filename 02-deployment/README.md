...
```bash
# ดู deployment
kubectl get deployment workshop-web -n workshop

# ดู pods ที่สร้างมาจาก deployment
kubectl get pods -n workshop -l app=workshop-web

# ดูสถานะ rollout
kubectl rollout status deployment/workshop-web -n workshop

# ดู ReplicaSet ที่ deployment สร้างขึ้น
kubectl get replicaset -n workshop
```
...
kubectl delete pod -l app=workshop-web -n workshop --wait=false
kubectl get pods -n workshop -w   # watch ดูว่า pod ใหม่ขึ้นมา

# Scale replicas
kubectl scale deployment workshop-web --replicas=4 -n workshop
kubectl get pods -n workshop

# Scale กลับ
kubectl scale deployment workshop-web --replicas=2 -n workshop
```
...
kubectl set image deployment/workshop-web web=nginx:1.26-alpine -n workshop

# ดูสถานะ rolling update
kubectl rollout status deployment/workshop-web -n workshop

# ดู history
kubectl rollout history deployment/workshop-web -n workshop

# Rollback
kubectl rollout undo deployment/workshop-web -n workshop
```
